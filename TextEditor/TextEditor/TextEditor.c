/*
 * Advanced Mini-Kilo Text Editor for Windows (MSVC)
 * Features: Syntax Highlighting, Search, Line Numbers
 * Command: cl editor_v2.c /O2
 */
...
char* C_HL_keywords[] = {
    ...
    "void|", "const|", "#define|", "#include|", NULL
};
...
void disableRawMode() {
    SetConsoleMode(E.hOut, E.origConsoleMode);
}

void enableRawMode() {
    E.hOut = GetStdHandle(STD_OUTPUT_HANDLE);
    if (E.hOut == INVALID_HANDLE_VALUE) die("GetStdHandle");
    if (!GetConsoleMode(E.hOut, &E.origConsoleMode)) die("GetConsoleMode");

    DWORD rawMode = E.origConsoleMode;
...
void editorOpen(char* filename) {
    ...
    FILE* fp;
    if (fopen_s(&fp, filename, "r") != 0 || !fp) die("fopen");
    ...
    while (fgets(buffer, sizeof(buffer), fp)) {
        ...
        editorInsertRow(E.numrows, buffer, len);
    }
    fclose(fp);
    E.dirty = 0;
}
...
void editorSave() {
    ...
    FILE* fp;
    if (fopen_s(&fp, E.filename, "w") == 0 && fp) {
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        if (E.show_line_numbers) {
            ...
            if (filerow < E.numrows)
                lnlen = snprintf(ln, sizeof(ln), "%4d ", filerow + 1);
            else
                lnlen = snprintf(ln, sizeof(ln), "     ");

            abAppend(ab, "\x1b[38;5;240m", 11); // Grey
            abAppend(ab, ln, lnlen);
...
void editorRefreshScreen() {
    ...
    // Adjust cursor for line numbers
    int gutter = E.show_line_numbers ? 5 : 0;
    char buf[32];
    ...
    case CTRL_KEY('q'):
        if (E.dirty && quit_times > 0) {
            editorSetStatusMessage("WARNING!!! Unsaved changes. "
                "Ctrl-Q %d more times to quit.", quit_times);
            quit_times--;
            return;
        }
        write(1, "\x1b[2J", 4);
...
int main(int argc, char* argv[]) {
    ...
    editorSetStatusMessage("HELP: Ctrl-S = save | Ctrl-Q = quit | Ctrl-F = find | Ctrl-N = nums");
    while (1) {
        editorRefreshScreen();
